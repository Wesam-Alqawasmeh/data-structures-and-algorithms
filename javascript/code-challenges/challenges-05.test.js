...
const toLastNames = (people) => {
  // Solution code here...
  let newList = people.map((item) => {
    let person = `${item.firstName} ${item.lastName}`;
    return person;
  });
  return newList;
};
...
const addValues = (arr) => {
  // Solution code here...
  let add = arr.reduce((accumulator, value) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((accomulator, value) => {
    accomulator.push(`${value.name}`);
    return accomulator;
  }, []);
  return newArr;
};
...
const reversedString = (str) => {
  ...
  let reverseString = arrayOfLetters.reduce((accomulator, value) => {
    accomulator = `${value}` + accomulator;
    return accomulator;
  }, "");
