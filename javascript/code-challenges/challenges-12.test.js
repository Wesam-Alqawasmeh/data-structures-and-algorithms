...
const validateEmail = (email) => {
  ...
  // let regex = /\b^(([a-zA-Z0-9.]+)\w*)+@([a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*)+.((net)|(com)|(org))$\b/;
  // let regex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@([a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*)+(.net | .com | .org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
let regex = /^[+]?(1\-|1\s|1|\d{3}\-|\d{3}\s|)?((\(\d{3}\))|\d{3})(\-|\s)?(\d{3})(\-|\s)?(\d{4})$/g ;
